...
class ZipRecruiterScraper(Scraper):
        ...
        try:
            ...
            if res.status_code not in range(200, 400):
                if res.status_code == 429:
                    logger.error(
                        f"429 Response - Blocked by ZipRecruiter for too many requests"
                    )
                else:
    ...
    def _process_job(self, job: dict) -> JobPost | None:
        ...
        return JobPost(
            ...
            compensation=Compensation(
                ...
                min_amount=(
                    int(job["compensation_min"]) if "compensation_min" in job else None
                ),
                max_amount=(
                    int(job["compensation_max"]) if "compensation_max" in job else None
                ),
    ...
    def _get_cookies(self):
        ...
        self.session.post(
            f"{self.api_url}/jobs-app/event", data=data, headers=self.headers
        )
    ...
    def _add_params(scraper_input) -> dict[str, str | Any]:
        ...
        if scraper_input.hours_old:
            fromage = (
                max(scraper_input.hours_old // 24, 1)
                if scraper_input.hours_old
                else None
            )
            params["days"] = fromage
        ...
        if scraper_input.job_type:
            params["employment_type"] = (
                job_type_map[scraper_input.job_type]
                if scraper_input.job_type in job_type_map
                else scraper_input.job_type.value[0]
            )
