...
class IndeedScraper(Scraper):
    """A class to scrape job postings from Indeed."""

    def __init__(self, proxy: str | None = None):
        """Initialize the IndeedScraper instance.

        Args:
            proxy (str | None, optional): Proxy settings. Defaults to None.
        """
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        """Scrape job postings based on the provided input.

        Args:
            scraper_input (ScraperInput): Input configuration for the scraper.
    ...
    def _scrape_page(self, page: int = 0) -> list[JobPost]:
        """Scrape job postings from a single page.

        Args:
            page (int, optional): Page number to scrape. Defaults to 0.

        Returns:
            list[JobPost]: List of scraped job postings.
        """
        job_list = []
    ...
    def _process_job(self, job: dict, job_detailed: dict) -> JobPost | None:
        ...
        Args:
            job (dict): Basic job information.
            job_detailed (dict): Detailed job information.

        Returns:
            JobPost | None: Processed job posting or None if already seen.
        """
    ...
    def _get_job_details(self, job_keys: list[str]) -> dict:
        """Get detailed information for the given job keys.

        Args:
            job_keys (list[str]): List of job keys.

        Returns:
            dict: Detailed information for the job keys.
        """
    ...
    def _add_params(self, page: int) -> dict[str, str | Any]:
        """Add parameters for the job search query.

        Args:
            page (int): Page number.

        Returns:
            dict[str, str | Any]: Parameters for the job search query.
        """
        fromage = (
    ...
    def _get_job_type(job: dict) -> list[JobType] | None:
        """Get job types from the job information.
    ...
    def _get_compensation(job: dict, job_detailed: dict) -> Compensation:
        """Get compensation information from the job information.
    ...
    def _parse_jobs(soup: BeautifulSoup) -> dict:
        """Parse job information from the BeautifulSoup object.
    ...
    def _is_job_remote(job: dict, job_detailed: dict, description: str) -> bool:
        """Check if the job is remote based on various criteria.
    ...
    def _get_correct_interval(interval: str) -> CompensationInterval:
        """Get the correct CompensationInterval from the provided string.

        Args:
            interval (str): Interval string.

        Returns:
            CompensationInterval: Corresponding CompensationInterval.
